...
|---|---|---|---|---|
| **v1.2.0** | 1 | 14.2.x | none yet — additive capability, no consumer repinned in this package (`netyvee/app#344` F2-B1); `Shell`'s output is byte-identical for every current consumer (full pre-existing suite green unmodified) | ✅ **current** |
...
| v0.6.1–v0.6.13 | 1 | 14.2.x | not reconstructed here — this matrix had already drifted to v0.6.0 "current" again (the same WF-GOV-01 pattern the note above describes) while `package.json` itself advanced to 0.6.13; see `CHANGELOG.md` for what shipped in each | superseded — matrix gap, not this package's to backfill |
| v0.6.0 | 1 | 14.2.x | **main** - `NavLink.rel` (closed allow-list) rendered as `data-vf-rel`; additive, output byte-identical for consumers that declare no `rel`. care/staffing NOT repinned - see SM-F2 coupling note | superseded (additive minor) |
